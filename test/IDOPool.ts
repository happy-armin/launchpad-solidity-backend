...
describe("IDOPool", () => {
  ...
  before(async () => {
    ...
    const startTimestamp =
      (await ethers.provider.getBlock("latest"))?.timestamp + 100; // start in the future
    ...
    rewardToken = await FeeTokenFactory.connect(deployer).deploy(
      "DogToken",
      "DTK",
    ...
    await rewardToken
      .connect(deployer)
      .transfer(idoPool, ethers.parseEther("1000"));
  });

  it("test stake function before starting", async () => {
    await expect(
      idoPool.connect(alice).stake(ethers.parseEther("100"))
    ).to.be.revertedWith("Project not started");
  });
  ...
  it("test refund function before ending", async () => {
    await expect(idoPool.connect(alice).refund()).to.be.revertedWith(
  ...
  it("test claim function before claiming", async () => {
    await expect(idoPool.connect(alice).claim()).to.be.revertedWith(
